...
from bundesrag.ingestion.pipeline import DeleteSummary, DownloadAborted, DownloadSummary, IndexSummary
...
def test_clear_command_skips_confirmation_with_yes_flag(settings, mocker):
    mocker.patch.object(cli, "Settings", return_value=settings)
    mocker.patch.object(cli, "get_vectorstore", return_value=mocker.Mock())
    run_delete_all = mocker.patch.object(cli, "run_delete_all", return_value=DeleteSummary(num_files=2))
